# Create Private Service Connection VPC
...
# Create subnets for Private Service Connection VPC, each subnet map to a region.
resource "google_compute_subnetwork" "psc_vpc_subnets" {
  for_each      = var.regional_config
  project       = var.project
  region        = each.key
  name          = "${var.psc_vpc_name}-subnet-${each.key}"
  ip_cidr_range = each.value.psc_vpc_subnet_ip_cidr_range
  network       = google_compute_network.psc_vpc.id
}

# Create cloud routers per subnet per region.
resource "google_compute_router" "psc_vpc_subnet_cloud_routers" {
  for_each = var.regional_config
  project  = var.project
  region   = each.key
  name     = "${var.psc_vpc_name}-cr-${each.key}"
  ...
  depends_on = [
    google_compute_subnetwork.psc_vpc_subnets
  ]
}
...
resource "google_compute_address" "cr_primary_addr" {
  for_each     = var.regional_config
  project      = var.project
  name         = "${var.psc_vpc_name}-${each.key}-cr-primary-addr"
  region       = google_compute_subnetwork.psc_vpc_subnets[each.key].region
  subnetwork   = google_compute_subnetwork.psc_vpc_subnets[each.key].id
  address_type = "INTERNAL"
  address      = cidrhost(each.value.psc_vpc_subnet_ip_cidr_range, (pow(2, (32 - tonumber(split("/", each.value.psc_vpc_subnet_ip_cidr_range)[1]))) - 4))
}
...
resource "google_compute_address" "cr_redundant_addr" {
  for_each     = var.regional_config
  project      = var.project
  name         = "${var.psc_vpc_name}-${each.key}-cr-redundant-addr"
  region       = google_compute_subnetwork.psc_vpc_subnets[each.key].region
  subnetwork   = google_compute_subnetwork.psc_vpc_subnets[each.key].id
  address_type = "INTERNAL"
  address      = cidrhost(each.value.psc_vpc_subnet_ip_cidr_range, (pow(2, (32 - tonumber(split("/", each.value.psc_vpc_subnet_ip_cidr_range)[1]))) - 3))
}
...
resource "google_compute_router_interface" "cr_redundant_interface" {
  for_each           = var.regional_config
  project            = var.project
  name               = "${google_compute_router.psc_vpc_subnet_cloud_routers[each.key].name}-int-redundant"
  region             = google_compute_router.psc_vpc_subnet_cloud_routers[each.key].region
  router             = google_compute_router.psc_vpc_subnet_cloud_routers[each.key].name
  subnetwork         = google_compute_subnetwork.psc_vpc_subnets[each.key].self_link
...
resource "google_compute_router_interface" "cr_primary_interface" {
  for_each            = var.regional_config
  project             = var.project
  name                = "${google_compute_router.psc_vpc_subnet_cloud_routers[each.key].name}-int-primary"
  region              = google_compute_router.psc_vpc_subnet_cloud_routers[each.key].region
  router              = google_compute_router.psc_vpc_subnet_cloud_routers[each.key].name
  subnetwork          = google_compute_subnetwork.psc_vpc_subnets[each.key].self_link
...
resource "google_sql_database_instance" "mysql_primary" {
  ...
  settings {
    ...
    ip_configuration {
      ipv4_enabled                                  = false
      private_network                               = google_compute_network.psc_vpc.id
...
module "mc-transit" {
  ...
  bgp_lan_interfaces = [{
    vpc_id = google_compute_network.psc_vpc.name
    subnet = each.value.psc_vpc_subnet_ip_cidr_range
  }]
...
resource "aviatrix_transit_external_device_conn" "bgp_over_lan" {
  ...
  connection_name           = google_compute_router.psc_vpc_subnet_cloud_routers[each.key].name
  connection_type           = "bgp"
  tunnel_protocol           = "LAN"
  ha_enabled                = true
